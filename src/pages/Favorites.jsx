...
import { hydrateFavoritesFromCache } from "../store/favoritesSlice";
...
export const Favorites = () => {
  ...
  // Fetch sólo si aún no se ha hecho la 1ª carga y no está cargando
  useEffect(() => {
...
useEffect(() => {
  ...
  // Primera pintura limpia
  const showTable = rows.length > 0;
  const showEmpty = hasFetchedOnce && !loading && rows.length === 0;
  ...
  {
    // Título (bold) arriba + artista debajo (gris), con truncado
    name: "",
    selector: (r) => r.title,
    grow: 3,
    sortable: true,
  ...
  // ❌ sin duración
  {
    // Acción fija a la derecha: siempre visible
    name: "",
    width: "60px",
    right: true,
    cell: (r) => (

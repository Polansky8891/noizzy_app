...
export const Register = () => {
  ...
  const validate = () => {
    ...
    else if (form.name.trim().length < 2) newErrors.name = 'First Name must be at least 2 characters';
    else if (!/^[A-Za-z]+$/.test(form.name)) newErrors.name = 'First Name can only contain letters';

    if (!form.lastName.trim()) newErrors.lastName = 'Last Name is required';
    else if (form.lastName.trim().length < 2) newErrors.lastName = 'Last Name must be at least 2 characters';
    else if (!/^[A-Za-z]+$/.test(form.lastName)) newErrors.lastName = 'Last Name can only contain letters';
    ...
    else if (!/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(form.password)) newErrors.password = 'Password must contain uppercase, lowercase, and a number';
      ...
      <div className="translate-y-[-12vh] w-full max-w-lg">
        <div className="bg-gray-300 rounded-xl shadow-md py-8 px-8">
          ...
          <form className="flex flex-col" onSubmit={handleSubmit}>
            {/* Nombres separados en móvil; en fila en desktop */}
            ...
            <div className="w-full mb-4">
              <input
                ...
                onChange={handleChange}
              />
              {errors.email && <span className="text-red-600 text-sm block mt-1 text-left">{errors.email}</span>}
            </div>
            ...
            <div className="w-full mb-4 relative">
              ...
              {errors.confirmPassword && (
                <span className="text-red-600 text-sm block mt-1 text-left">{errors.confirmPassword}</span>
              )}
            </div>

            <button
              className="bg-gradient-to-r from-indigo-500 to-blue-500 text-white py-2 rounded-md hover:from-indigo-600 hover:to-blue-600 transition ease-in duration-200 mt-2"
              type="submit"
            >
              Submit
            </button>
            ...
            <div className="flex justify-center items-center mt-2">
              ...
              <Link to="/login" className="text-sm px-2 no-underline text-blue-700">Sign In</Link>
            </div>
          </form>
        </div>
      </div>
